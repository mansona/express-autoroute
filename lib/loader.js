...
function loadDirectory(dirPath, prefix, app, options) {
    ...
    options.logger.debug("Current Folder: " + dirPath);
        ...
        files.forEach(function(path) {
            fs.stat(dirPath + "/" + path, function(err, stats) {
                ...
                if (stats.isDirectory()) {
                    // for directories add the directory to both the physical path, and virtual route
                    loadDirectory(dirPath + "/" + path, prefix + "/" + path, app, options);
                }
                else {
                    loadFile(dirPath + "/" + path, prefix, app, options);
                }
            });
        });
    });
